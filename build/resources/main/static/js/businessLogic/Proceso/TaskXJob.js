...
//todo: replace functions with list_select from jsForm.js
function  lis_dir_id(d, valor) {
    $.get("Direccion_Puesto", "opc=Listar_dir_dep&" + "id=" + valor, function (objJson) {
        d.empty();
        d.append("<option value='' > [Seleccione] </option>");
        if (objJson.rpta === -1) {
...
function list_dir(c) {
    $.get("Direccion_Puesto", "opc=getDirections", function (objJson) {
        c.empty();
        c.append("<option value='' > [Seleccione] </option>");
        // if (objJson.rpta === -1) {
        //     alert(objJson.mensaje);
...
function list_puesto(num) {
    ...
    $.get("steps/"+ $(".id_paso" + num).val()+"/jobs", function (objJson) {
        if (objJson.rpta === -1) {
            alert(objJson.mensaje);
            return;
        }
        ...
        $(".btn_deshabilitar_p").click(function () {
            $.post("steps", "opc=actualizar_estado&id=" + $(".iddp" + $(this).val()).val() + "&estado=0", function (objJson) {
                if (objJson.rpta === -1) {
                    alert(objJson.mensaje);
                    return;
                } else {
        ...
        $(".btn_habilitar_p").click(function () {
            $.post("steps", "opc=actualizar_estado&id=" + $(".iddp" + $(this).val()).val() + "&estado=1", function (objJson) {
                if (objJson.rpta === -1) {
                    alert(objJson.mensaje);
                    return;
                } else {
