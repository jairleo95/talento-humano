...
function Listar_Paso(proceso) {
    ...
    var TaskValuesHtml = "";
    TaskValues.empty();
    $.get("steps", "opc=Listar_habilitados&proceso=" + proceso, function (objJson) {
        c.hide();
        b.empty();
        c.empty();
        ...
        $(".btn-eliminar").click(
                function () {
                    ...
                    if (confirm("¿Esta Seguro de Eliminar?")) {
                        $.post("steps", "opc=Eliminar&paso=" + $(".id_paso" + $(this).val()).val(), function () {
                            Listar_Paso(pr_e);
                        });
...
function initFormTaskProcess() {
    ...
    $("#btn-registrar").click(function () {
        ...
        if ($(".form-paso").valid()) {
            $.post("steps", $(".form-paso").serialize() + "&proceso=" + pr, function (objJson) {
                if (objJson.rpta === "-1") {
                    alert(objJson.mensaje);
                    return;
                } else {
    ...
    $(".Generar").click(function () {
        ...
        $.post("steps", "opc=Update_nu_paso&num=" + num + "&" + url, function (objJson) {
            if (objJson.rpta === -1) {
                alert(objJson.mensaje);
                return;
            } else {
