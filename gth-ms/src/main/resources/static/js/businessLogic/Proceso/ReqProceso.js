...
//     $.get("requerimiento", "opc=Listar_req", function (objJson) {
//         req.empty();
//         if (objJson.rpta === -1) {
...
function insertDetalleReqProceso() {
    $.ajax({
        url: "Proceso/details", data: $(".formReqProceso").serialize() + "&proceso=" + $(".selectProceso").val() + "&opc=insertDetalleReqProceso", type: 'POST',
...
function initFormReqProceso() {
    ...
    list_select($(".proceso"), "Proceso/all", "");
    list_select($(".direccion"), "Direccion_Puesto", "opc=getDirections", null, null, "Direccion");
