...
$(document).ready(function () {
    ...
    function cargar_Dir() {
        ...
        $.post("Costo?opc=list_dir", function (objJson) {
    ...
    function cargar_dep(id, selec) {
        ...
        $.post("Costo?opc=list_dep&id=" + id, function (objJson) {
            var lista = objJson.lista;
            if (lista.length > 0) {
                $('.indep').empty();
    ...
    function cargar_ar(id, selec) {
        ...
        $.post("Costo?opc=list_ar&id=" + id, function (objJson) {
            var lista = objJson.lista;
            if (lista.length > 0) {
                $('.inarea').empty();
    ...
    function cargar_sec(id, selec) {
        ...
        if (id !== undefined) {
            $.post("Costo?opc=list_se&id=" + id, function (objJson) {
                var lista = objJson.lista;
                if (lista.length > 0) {
                    $('.inseccion').empty();
    ...
    function cargar_T(id) {
        var url = "";
        if (id != null) {
            url = "Costo?opc=list_ccosto&id=" + id;
        } else {
            url = "Costo?opc=list_ccosto";
        }
                ...
                $('.btnDel').click(function () {
                    ...
                    $.SmartMessageBox({
                        ...
                        if (ButtonPressed === "Si") {
                            ...
                            $.post("Costo?opc=del_cc&id_det_cc=" + id_det_cc, function () {
                                cargar_T();
                                $.smallBox({
                                    title: "Eliminar Centro de Costo",
        ...
        if ($('.btnSave').val() == 1) {
            $.SmartMessageBox({
                ...
                if (ButtonPressed === "Si") {
                    ...
                    $.post("Costo?opc=add_cc", data, function () {
                        cargar_T();
                        $('.inccc').val("");
                        $('.indcc').val("");
            ...
            $.SmartMessageBox({
                ...
                if (ButtonPressed === "Si") {
                    ...
                    $.post("Costo?opc=edit_cc", data, function () {
                        cargar_T();
                        $('.inccc').val("");
                        $('.indcc').val("");
                        $('.id_cc').val("");
            ...
            $.post("Costo?opc=Asignar_cc", $(".form_cc").serialize() , function () {
                cargar_T($(".id_cc").val());
            });
    ...
    function crear_T() {
        ...
        t += '         <td class="text-center semi-bold" width="20%">Acciones</td>';
        t += '     </tr>';
        t += ' </thead>';
