...
function pageEvents() {
    ...
    $(".ReqPlanilla").click(function () {
        $(".oldContent").hide();
        $(".newContent").show();
        loadURL("Requerimiento?opc=Planilla", $(".newContent"));
    });

    $(".repCargaAcademica").click(function () {
        $(".oldContent").hide();
        $(".newContent").show();
        loadURL("Reporte_Carga_Academica", $(".newContent"));
    });
...
function showPrivileges(){
    console.log("enter to showPrivileges function");
    $.ajax({
        url: 'menu/privileges',
        data: '',
        type: 'GET',
        success: function (objJson) {
            ...
            for (var i = 0; i < objJson.length; i++) {
                html +='<a href="#" data-href="'+objJson[i].di_url+'" class="privilegeLink" ><i class="'+objJson[i].ic_link+'"></i>'+objJson[i].no_link+'</a>';
            }
            $(".privilegesItems").append(html);

            $(".privilegeLink").click(function () {
...
function showElements(){
    $.ajax({
        ...
        type: 'GET',
        success: function (objJson) {
            $(".breadcrumb").append(objJson.breadcrumb);
            $(".spanuser").append(objJson.spanuser);
            $(".fullname").append(objJson.fullname);
        }});
}
