...
function lis_dep(b) {
    $.get("Direccion_Puesto", "opc=Listar_direccion", function (objJson) {
        b.empty();
        if (objJson.rpta == -1) {
...
function  lis_dir_id(d, valor) {
    d.empty();
    if (valor != 0) {
        $.get("Direccion_Puesto", "opc=Listar_dir_dep&" + "id=" + valor.trim(), function (objJson) {
            if (objJson.rpta == -1) {
                alert(objJson.mensaje);
                return;
            }
...
function list_area_id(c, valor) {
    c.empty();
    if (valor != 0) {
        $.get("Direccion_Puesto", "opc=Listar_area&" + "id_dep=" + valor, function (objJson) {
            if (objJson.rpta == -1) {
                alert(objJson.mensaje);
                return;
            }
...
function list_sec_id(d, valor) {
    d.empty();
    if (valor != 0) {
        $.get("Direccion_Puesto", "opc=Listar_sec&" + "id_are=" + valor, function (objJson) {
            if (objJson.rpta == -1) {
                alert(objJson.mensaje);
                return;
            }
...
function list_puesto_id(e, valor) {
    e.empty();
    if (valor != 0) {
        $.get("Direccion_Puesto", "opc=Listar_pu_id&" + "id=" + valor, function (objJson) {
            if (objJson.rpta == -1) {
                alert(objJson.mensaje);
                return;
            }
...
function mostrar_plantilla(valor) {
    var editor = CKEDITOR.instances.editor1;
    $.get("templates", "opc=Listar&id=" + valor, function (objJson) {
        if (objJson.rpta == -1) {
            alert(objJson.mensaje);
            return;
        }
...
function Listar_Plantilla() {
    ...
    var texto = "";
    $.get("templates", "opc=List_Plamtillas&" + "id_dir=" + $(".dir").val() + "&id_dep=" + $(".dep").val() + "&id_are=" + $(".area").val() + "&sec=" + $(".seccion").val() + "&id_pu=" + $(".puesto").val(), function (objJson) {
        d.empty();
        if (objJson.rpta == -1) {
            alert(objJson.mensaje);
            return;
...
function list_dir_fil(c, fil, op) {
    $.get("Direccion_Puesto", "opc=Listar_direccion_filial&id_fil=" + fil.val(), function (objJson) {
        c.empty();
        if (objJson.rpta == -1) {
            alert(objJson.mensaje);
            return;
...
$(document).ready(function () {
    ...
    $(".nombre_pl").keyup(function () {
        ...
        if ($(".id_pl").val() != "" & $(this).val() != "") {
            $.ajax({
                url: "templates", type: 'POST', data: "nom_pl=" + $(this).val() + "&id=" + $(".id_pc").val() + "&opc=UpdateNameFile", success: function (data, textStatus, jqXHR) {
                    Listar_Plantilla();
